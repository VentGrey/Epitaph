...
my $reminders_file =
    "$ENV{HOME}/.config/leftwm/themes/Epitaph/calendar/reminders" ;
...
sub add_reminder {
    ...
    my $time_dialog = Gtk3::Dialog->new(
                                        'Set Reminder Time', $window, 'modal',
                                        'gtk-ok'     => 'accept',
                                        'gtk-cancel' => 'cancel',
    ) ;
    ...
    if ($time_dialog->run eq 'accept') {
        ...
        if ($dialog->run eq 'accept') {
            ...
            open(my $fh, '>>', $reminders_file)
                or die "Could not open file '$reminders_file' $!" ;
            ...
            my $message_dialog =
                ...
                'info',
                'ok',
                "Reminder created successfully at:  $day/$month/$year $hour:$minute"
                ) ;
            $message_dialog->run ;
    ...
    sub delete_reminders {
        ...
        my $label = Gtk3::Label->new(
                      "Do you want to delete all reminders or only old ones?") ;
        ...
        if ($response eq 'delete_all') {
            ...
            while (my $line = <$fh>) {
                ...
                push @current_reminders, $line
                    if $reminder_datetime >= $current_datetime ;
            }
            close $fh ;

            open($fh, '>', $reminders_file)
                or die "Could not open file '$reminders_file' $!" ;
            print $fh join("\n", @current_reminders) ;
            close $fh ;
        }
